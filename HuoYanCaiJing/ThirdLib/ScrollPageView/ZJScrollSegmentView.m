...
// 缓存计算出来的每个标题的宽度
@property (nonatomic, strong) NSMutableArray *titleStringWidth;


@property (strong,nonatomic) NSMutableArray *coverWidth;


// 响应标题点击
@property (copy, nonatomic) TitleBtnOnClickBlock titleBtnOnClick;

@property (nonatomic, strong) ZJCustomLabel *tmpLabel;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        if (index == 0) {
            label.font = self.segmentStyle.selectedTitleFont;
        }else{
...
//        label.font = self.segmentStyle.titleFont;
        ...
        if (self.segmentStyle.perWidth != 0) {
            [self.titleWidths addObject:@(self.segmentStyle.perWidth)];
            if (self.segmentStyle.autoScrollLineWidth) {
                ...
                [self.titleStringWidth addObject:@(bounds.size.width)];
            }
        ...
        if (self.segmentStyle.autoCoverWidth) {
            CGRect bounds = [title boundingRectWithSize:CGSizeMake(MAXFLOAT, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: label.font} context:nil];
            [self.coverWidth addObject:@(bounds.size.width + self.segmentStyle.titleMargin)];
        }
...
- (void)setUpLabelsPosition {
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        titleW = (_currentWidth - self.segmentStyle.leftMargin  - self.segmentStyle.rightMargin) / self.titles.count;
        ...
        for (ZJCustomLabel *label in self.titleLabels) {
            ...
            if (index == 0) {
                label.frame = CGRectMake(titleX + self.segmentStyle.xPlus + self.segmentStyle.leftMargin, titleY, titleW, titleH);
            } else {
                label.frame = CGRectMake(titleX + self.segmentStyle.leftMargin, titleY, titleW, titleH);
            }
        ...
        for (ZJCustomLabel *label in self.titleLabels) {
            ...
            if (index == 0) {
                label.frame = CGRectMake(self.segmentStyle.titleCoverLeftMargin + titleX * 0.5 + self.segmentStyle.xPlus , titleY, titleW, titleH);
            } else {
                label.frame = CGRectMake(titleX + self.segmentStyle.leftMargin, titleY, titleW, titleH);
            }
            index++;
...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.autoScrollLineWidth) {
            ...
            CGFloat width = [self.titleStringWidth[0] floatValue];
            
            self.scrollLine.frame = CGRectMake(coverX, self.zj_height - self.segmentStyle.scrollLineHeight, width , self.segmentStyle.scrollLineHeight);
        } else {
            self.scrollLine.frame = CGRectMake(coverX + (self.segmentStyle.perWidth -  self.segmentStyle.scrollVLineWidth) / 2, self.zj_height - self.segmentStyle.scrollLineHeight, self.segmentStyle.scrollVLineWidth , self.segmentStyle.scrollLineHeight);
        }
    ...
    if (self.coverLayer) {
        CGFloat w = WIDTH / 4.0;
        CGFloat margin = (self.segmentStyle.perWidth - self.segmentStyle.coverWidth) / 2;
        
        if (self.segmentStyle.isScrollTitle) {
            if (self.segmentStyle.autoCoverWidth) {
                CGFloat width = [self.coverWidth[0] floatValue];
                if (self.segmentStyle.titleCoverMargin == 0) {
                    self.coverLayer.frame = CGRectMake(coverX - xGap - self.segmentStyle.titleMargin * 0.5, self.bounds.size.height - 2, width, coverH);
                }else{
                    self.coverLayer.frame = CGRectMake(coverX - xGap - self.segmentStyle.titleMargin * 0.5 + self.segmentStyle.titleCoverMargin, self.bounds.size.height - 2, width - self.segmentStyle.titleCoverMargin * 2, coverH);
                }
            } else {
                self.coverLayer.frame = CGRectMake(coverX - xGap + margin, self.bounds.size.height - 2, self.segmentStyle.coverWidth, coverH);
            }
           
            
        } else {
            self.coverLayer.frame = CGRectMake(coverX + margin, self.bounds.size.height - 2, self.segmentStyle.coverWidth, coverH);
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            ...
            if (self.segmentStyle.autoScrollLineWidth) {
                CGFloat width = [self.titleStringWidth[_currentIndex] floatValue];
                weakSelf.scrollLine.zj_x = currentLabel.zj_x;
                weakSelf.scrollLine.zj_width = width;
            } else {
                weakSelf.scrollLine.zj_x = currentLabel.zj_x + (self.segmentStyle.perWidth -  self.segmentStyle.scrollVLineWidth) / 2;
                weakSelf.scrollLine.zj_width = self.segmentStyle.scrollVLineWidth;
            }
        ...
        if (weakSelf.coverLayer) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                if (self.segmentStyle.autoCoverWidth) {
                    CGFloat width = [self.coverWidth[_currentIndex] floatValue];
                    if (self.segmentStyle.titleCoverMargin == 0) {
                        weakSelf.coverLayer.zj_x = currentLabel.zj_x - xGap - self.segmentStyle.titleMargin * 0.5;
                        weakSelf.coverLayer.zj_width = width;
                    }else{
                        weakSelf.coverLayer.zj_x = currentLabel.zj_x - xGap - self.segmentStyle.titleMargin * 0.5 + self.segmentStyle.titleCoverMargin;
                        weakSelf.coverLayer.zj_width = width - self.segmentStyle.titleCoverMargin * 2;
                    }
                    
                } else {
                    weakSelf.coverLayer.zj_x = currentLabel.zj_x - xGap + margin;
                    weakSelf.coverLayer.zj_width = self.segmentStyle.coverWidth;
                }
                
               
            } else {
                weakSelf.coverLayer.zj_x = currentLabel.zj_x + margin;
                weakSelf.coverLayer.zj_width = self.segmentStyle.coverWidth;
            }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.autoScrollLineWidth) {
            CGFloat width = [self.titleStringWidth[currentIndex] floatValue];
            self.scrollLine.zj_x = oldLabel.zj_x + xDistance * progress + (self.segmentStyle.perWidth - width) / 2;
            self.scrollLine.zj_width = width;
        } else {
            self.scrollLine.zj_x = oldLabel.zj_x + xDistance * progress + (self.segmentStyle.perWidth -  self.segmentStyle.scrollVLineWidth) / 2;
            self.scrollLine.zj_width = self.segmentStyle.scrollVLineWidth;
        }
    ...
    if (self.coverLayer) {
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.autoCoverWidth) {
                CGFloat width = [self.coverWidth[currentIndex] floatValue];
                if (self.segmentStyle.titleCoverMargin == 0) {
                    self.coverLayer.zj_x = oldLabel.zj_x + xDistance * progress - xGap - self.segmentStyle.titleMargin * 0.5;
                    self.coverLayer.zj_width = width + wDistance * progress + wGap;

                }else{
                    self.coverLayer.zj_x = oldLabel.zj_x + xDistance * progress - xGap - self.segmentStyle.titleMargin * 0.5 + self.segmentStyle.titleCoverMargin;
                    self.coverLayer.zj_width = width + wDistance * progress + wGap - self.segmentStyle.titleCoverMargin*2;

                }
                
                
            } else {
                self.coverLayer.zj_x = oldLabel.zj_x + xDistance * progress - xGap + margin;
                self.coverLayer.zj_width = self.segmentStyle.coverWidth + wDistance * progress + wGap;

            }
            
            } else {
            self.coverLayer.zj_x = oldLabel.zj_x + xDistance * progress + margin;
            self.coverLayer.zj_width = self.segmentStyle.coverWidth + wDistance * progress;
        }
    }
...
- (NSMutableArray *)titleStringWidth {
    if (_titleStringWidth == nil) {
        _titleStringWidth = [NSMutableArray array];
    }
    return _titleStringWidth;
}

- (NSMutableArray *)coverWidth {
    if (!_coverWidth) {
        _coverWidth = [NSMutableArray array];
    }
    return _coverWidth;
}
