...
@property (weak, nonatomic) XZFBaseViewController *parentViewController;
...
- (void)commonInit {
    ...
    for (XZFBaseViewController *childVc in self.childVcs) {
...
- (void)setSelectedIndex:(NSInteger)selectedIndex animated:(BOOL)animated {
    ...
    self.contentView.indexDelegate = self.scrollSegmentDelegate;
...
- (ZJScrollSegmentView *)segmentView {
    ...
    _segmentView.titleButtonClicked = ^(NSInteger index){
        if (weakSelf.titleBtnOnClicked) {
            weakSelf.titleBtnOnClicked(index);
        }
    };
    return _segmentView;
}
