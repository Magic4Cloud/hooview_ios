...
@implementation LLImagePickerManager

+ (instancetype)manager{
    static LLImagePickerManager *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [LLImagePickerManager new];
    });
    return instance;
}
...
- (void)getVideoPathFromURL: (NSURL *)videoURL PHAsset: (PHAsset *)asset enableSave: (BOOL)enableSave completion: (void(^)(NSString *name, UIImage *screenshot, id pathData))completion {
    //视频名
    NSString *fileName = [self getMediaNameWithPHAsset:asset extensionName:@"Video.mp4"];
    //视频本地路径
...
- (UIImage *)imageWithVideoURL: (NSURL *)videoURL enableSave: (BOOL)enableSave {
    ...
    if (error) {
        LLLog(@"截取视频图片失败:%@",error.localizedDescription);
    }
