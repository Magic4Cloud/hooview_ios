...
@interface LLImagePickerView ()<UICollectionViewDelegate,UICollectionViewDataSource,TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
...
- (void)configureCollectionView {
    ...
    layout.itemSize = CGSizeMake((WIDTH - 40)/countOfRow, (WIDTH - 40)/countOfRow);
    layout.minimumLineSpacing = 0;
    layout.minimumInteritemSpacing = 0;
    layout.sectionInset = UIEdgeInsetsMake(0, 0, 0, 0);
    _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(40, 0, WIDTH - 40, HEIGHT) collectionViewLayout:layout];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    NSString * chooseImageTpye = [[NSUserDefaults standardUserDefaults] objectForKey:@"chooseImageTpye"];
    if ([chooseImageTpye isEqualToString:@"chooseVideo"]) {
        if (_mediaArray.count > 0 && indexPath.row == 1) {
            [UIAlertController showAlertWithTitle:@"最多选择一个视频" message:nil actionTitles:@[@"确定"] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
            return;
        }
    }
...
//                [alert show];
                ...
                UIAlertAction *takingPicturesAction = [UIAlertAction actionWithTitle:@"拍照" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    [weakSelf openCamera];
                }];
                UIAlertAction *picturAction = [UIAlertAction actionWithTitle:@"相册" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    [weakSelf openAlbum];
                }];
                [alertController addAction:cancelAction];
                [alertController addAction:takingPicturesAction];
                [alertController addAction:picturAction];
            ...
            default:
            {
                ...
                UIAlertAction *openVideotapeAction = [UIAlertAction actionWithTitle:@"录像" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                [takingPicturesAction setValue:UIColorFromHex(0x000000) forKey:@"_titleTextColor"];
                [picturAction setValue:UIColorFromHex(0x000000) forKey:@"_titleTextColor"];
                [openVideotapeAction setValue:UIColorFromHex(0x000000) forKey:@"_titleTextColor"];
                [videoAction setValue:UIColorFromHex(0x000000) forKey:@"_titleTextColor"];
                ...
                [alertController addAction:takingPicturesAction];
                [alertController addAction:picturAction];
                [alertController addAction:openVideotapeAction];
        ...
        for (LLImagePickerModel *model in _mediaArray) {
            NSLog(@"====898989===%@",model.image);
            if (model.isVideo) {
                if (model.mediaURL) {
                    MPMoviePlayerViewController *mPMoviePlayerViewController;
                    //            2.把下面的代码放到跳转播放的按钮方法里面
                    mPMoviePlayerViewController = [[MPMoviePlayerViewController alloc]initWithContentURL:model.mediaURL];
                    mPMoviePlayerViewController.view.frame = CGRectMake(0, 0, WIDTH, HEIGHT);
...
//            NSLog(@"----111----%@",_photos);
        ...
        DJEditPhotoViewController *djeVC = [[DJEditPhotoViewController alloc]init];
        djeVC.images = _photos;
        djeVC.currentIndex = indexPath.row;
        djeVC.hiddenDelete = YES;
        [[self viewController] presentViewController:djeVC animated:YES completion:nil];
    }
}
...
- (void)layoutCollection {
    ...
    NSString *str = [NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"XZFTeacherNoticeViewController"]];
    
    if ([str isEqualToString:@"XZFTeacherNoticeViewController"]) {
        [[NSNotificationCenter defaultCenter]postNotificationName:@"TeacherNoticeChangeTableViewUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }else if ([str isEqualToString:@"XZFSetNewPaymentViewController"]){
        [[NSNotificationCenter defaultCenter]postNotificationName:@"SetNewPaymentChangeTableViewUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }else if ([str isEqualToString:@"XZFHobbiesGroupSendViewController"]){
        [[NSNotificationCenter defaultCenter]postNotificationName:@"HobbiesGroupSendChangeTableViewUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }else if ([str isEqualToString:@"XZFHobbiesGroupSendNoticeViewController"]){
        [[NSNotificationCenter defaultCenter]postNotificationName:@"HobbiesGroupSendNoticeChangeTableViewUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }else if ([str isEqualToString:@"XZFOneToOneSendViewController"]){
        [[NSNotificationCenter defaultCenter]postNotificationName:@"XZFOneToOneSendChangeTableViewUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }else if ([str isEqualToString:@"SubmitTopicViewController"]){
        [[NSNotificationCenter defaultCenter]postNotificationName:@"SubmitTopicViewControllerChangeUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }else if ([str isEqualToString:@"SubChildViewController"]){
        [[NSNotificationCenter defaultCenter]postNotificationName:@"SubChildViewControllerChangeUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }else if ([str isEqualToString:@"SubJoinGameViewController"]){
        [[NSNotificationCenter defaultCenter]postNotificationName:@"SubJoinGameViewControllerChangeUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
    }
    
    
//    [[NSNotificationCenter defaultCenter]postNotificationName:@"ChangeTableViewUI" object:nil userInfo:@{@"count":@(_mediaArray.count)}];
...
- (void)openVideotape {
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        picker.videoMaximumDuration = 120.0f; //录像最长时间
    } else {
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"chooseImage" forKey:@"chooseImageTpye"];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if ([mediaType isEqualToString:@"public.movie"]) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:@"chooseVideo" forKey:@"chooseImageTpye"];
