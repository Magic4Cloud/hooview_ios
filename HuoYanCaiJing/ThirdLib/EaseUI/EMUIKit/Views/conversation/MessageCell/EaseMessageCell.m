...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model
{
    ...
    EaseMessageCell *cell = [self appearance];
    CGFloat bubbleMaxWidth = WIDTH - 150;
...
//    CGFloat height = EaseMessageCellPadding + cell.bubbleMargin.top + cell.bubbleMargin.bottom;
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText:
        {
            ...
            NSLog(@"----11111111000--rect-----%f",rect.size.height);

            
            height += (rect.size.height > 20 ? rect.size.height : 20);
            
            NSLog(@"----000009999911111111000--height-----%f",height);
