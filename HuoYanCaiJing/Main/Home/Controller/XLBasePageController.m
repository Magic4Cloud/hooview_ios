...
-(UIScrollView *)titleBackground
{
    if (!_titleBGScroll) {
        ...
        _titleBGScroll.backgroundColor = [UIColor clearColor];
...
-(void)scrollViewOffset:(UIButton *)button
{
    ...
    if (CGRectGetMidX(button.frame)>CGRectGetMidX(self.view.frame)) {
        if (_titleBGScroll.contentSize.width<CGRectGetMaxX(self.view.frame)/2+CGRectGetMidX(button.frame)) {
            [_titleBGScroll setContentOffset:CGPointMake(_titleBGScroll.contentSize.width-CGRectGetWidth(self.view.frame) + 100, 0) animated:YES];
        }
        else{
            [_titleBGScroll setContentOffset:CGPointMake(CGRectGetMidX(button.frame)-CGRectGetWidth(self.view.frame)/2 + 100, 0) animated:YES];
        }
    }
    else{
...
-(void)viewDidLayoutSubviews
{
    ...
    _titleBGScroll.frame = CGRectMake(50, (headerView.frame.size.height)?headerView.frame.origin.y+headerView.frame.size.height:self.topLayoutGuide.length, self.view.frame.size.width - 100,[self.dataSource respondsToSelector:@selector(heightForTitleViewPager:)]?[self.dataSource heightForTitleViewPager:self]:0);
