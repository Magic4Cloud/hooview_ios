//
//  HYCJMainMyAttentionViewController.m
//  HuoYanCaiJing
//
...
@interface HYCJMainMyAttentionViewController ()<UITableViewDataSource,UITableViewDelegate>{
    NSInteger page;
}
...
@implementation HYCJMainMyAttentionViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model) {
        ...
        if (self.dataArray.count > 0) {
            ...
            [cell.attentionButton setEnabled:NO];
            if (mutual == 1) {
                ...
                [cell.attentionButton setTitleColor:RGB(155, 155, 155, 1) forState:UIControlStateNormal];
                [cell.attentionButton setImage:[UIImage imageNamed:@"icon_fans_follow"] forState:UIControlStateNormal];
...
- (void)loadUserMyfollowOrMyfansData{
    ...
    [XZFHttpManager GET:UserMyfollowUrl parameters:@{@"page":@(page),@"limit":@(10),@"userId":userId} requestSerializer:NO requestToken:YES success:^(id respondseObject) {
        NSLog(@"--------我的关注--------%@",respondseObject);
        if (page == 1) {
            [self.dataArray removeAllObjects];
        }
        NSArray *arr = respondseObject[@"myFollowList"][@"list"];
        if (arr.count == 0) {
        ...
        [self.dataArray addObjectsFromArray:[HYCJMyAttentionModel mj_objectArrayWithKeyValuesArray:respondseObject[@"myFollowList"][@"list"]]];
