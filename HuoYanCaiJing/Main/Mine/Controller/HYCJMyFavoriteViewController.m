//
//  HYCJMyFavoriteViewController.m
//  HuoYanCaiJing
//
...
@interface HYCJMyFavoriteViewController (){
    NSInteger indexType;
    NSInteger page;
}
...
@implementation HYCJMyFavoriteViewController
...
- (NSArray *)setupChildVcAndTitle {
    NSArray *controllerStr = @[@"HYCJMyFavoriteLiveViewController",@"HYCJMyFavoriteFinanceViewController"];
    NSArray *titles = @[@"直播",@"财经"];
    NSMutableArray *controllers = [NSMutableArray array];
    for (NSInteger index = 0; index < controllerStr.count; index ++) {
        Class class = NSClassFromString(controllerStr[index]);
        HYCJMyFavoriteLiveViewController *homeListVc = [[class alloc] init];
