...
@property(nonatomic, weak)MainTabBarButton *selectedTwoButton;
...
//        [self SetupWriteButton];
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(ChooseButtonNotification:) name:@"ChooseButtonNotification" object:nil];
    }
...
//        }
        if (nIndex == 1) {
            self.selectedTwoButton = tabBarBtn;
        }
...
- (void)ChooseButtonNotification:(NSNotification *)notification{
    self.selectedButton.selected = NO;
    _selectedTwoButton.selected = YES;
    self.selectedButton = _selectedTwoButton;
}
