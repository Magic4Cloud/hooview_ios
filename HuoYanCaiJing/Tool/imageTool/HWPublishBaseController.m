...
//    addImageStrLabel.font = [UIFont systemFontOfSize:14];
...
{
    return CGSizeMake(([UIScreen mainScreen].bounds.size.width-40) /4,([UIScreen mainScreen].bounds.size.width-40) /4);
}
...
- (void)deletePhoto:(UIButton *)sender{
    ...
    [self changeCollectionViewHeight];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"chooseImageEndNotification" object:nil];
...
- (void)changeCollectionViewHeight{
    ...
    if (_collectionFrameY) {
        _pickerCollectionView.frame = CGRectMake(0, _collectionFrameY, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-40.0) /3.0 +20.0)* ((int)(_arrSelected.count)/3 +1)+20.0);
    }
    else{
        _pickerCollectionView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-40.0) /3.0 +20.0)* ((int)(_arrSelected.count)/3 +1)+20.0);
    }
 ...
 *  相册完成选择得到图片
 */
-(void)getSelectImageWithALAssetArray:(NSArray *)ALAssetArray thumbnailImageArray:(NSArray *)thumbnailImgArray commonImgArr:(NSArray *)commonImgArr{
    //（ALAsset）类型 Array
    _arrSelected = [NSMutableArray arrayWithArray:ALAssetArray];
    //正方形缩略图 Array
    _imageArray = [NSMutableArray arrayWithArray:thumbnailImgArray] ;
    
//    _commonImageArray = [NSMutableArray array];
    _commonImageArray = [NSMutableArray arrayWithArray:commonImgArr];
    
    
    CGFloat sizeHeight = ([UIScreen mainScreen].bounds.size.width-40) /3;
    NSInteger i;
    NSLog(@"%ld",self.arrSelected.count);
    NSInteger x = self.arrSelected.count + 1;
    if ( x > 3) {
        i = 2;
    }else{
        i = 1;
    ...
    if (self.arrSelected.count == 0) {
        ...
        height = (sizeHeight + 15) * i;
    }
    self.pickerCollectionView.frame = CGRectMake(0, 0, WIDTH, height);
...
- (void)updatePickerViewFrameY:(CGFloat)Y{
    ...
    _collectionFrameY = Y;
    _pickerCollectionView.frame = CGRectMake(0, Y, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-40.0) /3.0 +20.0)* ((int)(_arrSelected.count)/3 +1)+20.0);
}
