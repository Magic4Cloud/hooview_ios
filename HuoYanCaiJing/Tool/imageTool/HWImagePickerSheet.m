...
- (void)finishSelectImg{
    ...
    NSMutableArray *commonImgArr = [NSMutableArray array];

    for (ALAsset *set in _arrSelected) {
        CGImageRef cgImg = [set thumbnail];
        UIImage* image = [UIImage imageWithCGImage: cgImg];
        [thumbnailImgArr addObject:image];
        UIImage *tempImg=[UIImage imageWithCGImage:set.defaultRepresentation.fullScreenImage];
        [commonImgArr addObject:tempImg];

    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(getSelectImageWithALAssetArray:thumbnailImageArray: commonImgArr:)]) {
        [self.delegate getSelectImageWithALAssetArray:_arrSelected thumbnailImageArray:thumbnailImgArr commonImgArr:commonImgArr];
    }
}
