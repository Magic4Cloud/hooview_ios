...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            UIButton *dissmiss = [UIButton buttonWithType:UIButtonTypeCustom];
            [dissmiss setImage:[UIImage imageNamed:@"back"] forState:UIControlStateNormal];
            [dissmiss addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
            dissmiss.frame = CGRectMake(0, 0, 50, 85);
...
//    _cropRect = CGRectMake(self.view.tz_width / 2 - circleCropRadius, self.view.tz_height / 2 - _circleCropRadius, _circleCropRadius * 2, _circleCropRadius * 2);
    _cropRect = CGRectMake(0, (self.view.tz_height- self.view.tz_width*2/3.0)/2, self.view.tz_width, self.view.tz_width*2/3.0);
...
- (CGRect)cropRect {
    ...
    return CGRectMake(0, (self.view.tz_height - cropViewWH*2/3.0) / 2, cropViewWH, cropViewWH*2/3.0);
}
