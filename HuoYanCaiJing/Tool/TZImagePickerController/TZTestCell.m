...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _videoImageView.image = [UIImage imageNamedFromMyBundle:@"videoPlayer"];
        _videoImageView.contentMode = UIViewContentModeScaleAspectFill;
        _videoImageView.hidden = YES;
        [self addSubview:_videoImageView];
        
        _deleteBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_deleteBtn setImage:[UIImage imageNamed:@"photo_delegate"] forState:UIControlStateNormal];
        _deleteBtn.frame = CGRectMake(self.tz_width - 22, 0, 22, 22);
...
- (void)layoutSubviews {
    ...
    _imageView.frame = CGRectMake(0, 10, self.bounds.size.width-10, self.bounds.size.height-10);
    
    CGFloat width = self.tz_width / 4.0;
    _videoImageView.frame = CGRectMake(width-5, width+5, 2*width, 2*width);
}
