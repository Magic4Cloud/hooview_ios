...
- (void)configToolView {
    ...
    self.toolView.frame = CGRectMake(0, 0, WIDTH, 44*KWidth_Scale);
    [self addSubview:self.toolView];
    
    UIButton *saveBtn = [[UIButton alloc] init];
    saveBtn.frame = CGRectMake(WIDTH - 50*KWidth_Scale, 2*KWidth_Scale, 40*KWidth_Scale, 40*KWidth_Scale);
    [saveBtn setTitle:@"确认" forState:UIControlStateNormal];
    [saveBtn setTitleColor:UIColorFromHex(0x999999) forState:UIControlStateNormal];
    saveBtn.titleLabel.font = [UIFont pingfangFontWithSize:15*KWidth_Scale];
    [saveBtn addTarget:self action:@selector(saveBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.toolView addSubview:saveBtn];
    
    UIButton *cancelBtn = [[UIButton alloc] init];
    cancelBtn.frame = CGRectMake(10*KWidth_Scale, 2*KWidth_Scale, 40*KWidth_Scale, 40*KWidth_Scale);
    [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    [cancelBtn setTitleColor:UIColorFromHex(0x999999) forState:UIControlStateNormal];
    cancelBtn.titleLabel.font = [UIFont pingfangFontWithSize:15*KWidth_Scale];
    [cancelBtn addTarget:self action:@selector(cancelBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.toolView addSubview:cancelBtn];
    
    self.titleLbl = [[UILabel alloc] init];
    self.titleLbl.frame = CGRectMake(60*KWidth_Scale, 2*KWidth_Scale, WIDTH - 120*KWidth_Scale, 40*KWidth_Scale);
    self.titleLbl.textAlignment = NSTextAlignmentCenter;
    self.titleLbl.textColor = UIColorFromHex(0x2EA2FD);
    self.titleLbl.font = [UIFont pingfangFontWithSize:18*KWidth_Scale];
    [self.toolView addSubview:self.titleLbl];
}

/// 配置UIPickerView
- (void)configPickerView {
    self.pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 40 * KWidth_Scale, WIDTH, self.frame.size.height - 50*KWidth_Scale)];
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    UILabel *titleLbl = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width, 30*KWidth_Scale)];
    titleLbl.font = [UIFont pingfangFontWithSize:18*KWidth_Scale];
    titleLbl.textAlignment = NSTextAlignmentCenter;
    titleLbl.textColor = UIColorFromHex(0x333333);
